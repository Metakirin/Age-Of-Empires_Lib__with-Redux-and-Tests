...
export const MainNavLink = ({
  ...
  const handleClick = () => {
    switch (path) {
      case paths.civilizations:
        return getCivilizationsAction(path)
      case paths.units:
        return getUnitsAction(path)
      case paths.structures:
        return getStructuresAction(path)
      case paths.technologies:
        return getTechnologiesAction(path)
    }
  }
  return (
    <div className="menu_link">
      <NavLink activeClassName="nav_active" onClick={handleClick} to={path}>
        {title}
      </NavLink>
    </div>
  )
...
const mapDispatchToProps = (dispatch) => ({
  getCivilizationsAction: (path) =>
    dispatch(ACTION_GET_CIVILIZATIONS_DATA_REQUESTED(path)),
  getUnitsAction: (path) => dispatch(ACTION_GET_UNITS_DATA_REQUESTED(path)),
  getStructuresAction: (path) =>
    dispatch(ACTION_GET_STRUCTURES_DATA_REQUESTED(path)),
  getTechnologiesAction: (path) =>
    dispatch(ACTION_GET_TECHNOLOGIES_DATA_REQUESTED(path)),
})
